...
Filecoin is becoming a fascinating playground for people collaboratively coming to consensus around a public decentralized censorship resistant storage market, turning cloud storage into an algorithmic market.

Our team feel an average joe (not familiar with blockchains) is not aware to the impact Filecoin is going to generate. And we want to share our enthusiasm with the community at large.

Noticing the above undercurrent, we feel the need for to create experiences that give a feeling akin to the [feeling one get's when watching a rocket launch](https://www.youtube.com/watch?v=nqOU2CGeAvk) 🚀

Therefore, we propose to build an umbrella project that specifically deals with visualization tools over Filecoin that lets end users sit back and glimpse through the new universe of markets and applications that are going to be created with/on Filecoin.
...
The proposal is structured into 2 broad category of deliverables:
...
2. Marketing and Education deliverables.
   1. FILViz bot: A twitter bot that acts as a snapshot service over the above visualizations. Relays important updates on the Filecoin network to Twitter.
...
1. Macro view of the evolution of the Filecoin miner ecosystem. We were inspired by the visualizations in the intro video for Filecoin here and we want to build a tool that does this in real time.

- Miner activity being mapped on the globe. This viz would be similar to:
  - https://blueshift.io/international-trade.html ( we envision visualizing deal transfers like this)
  - https://blocks.wizb.it/ (blocks being mined)
  - https://foamviz.surge.sh/#/data-globe (evolution of the network with a time slider, people can scrub back and forth and view the network evolving from a time machine perspective)

##### 2. Visualizing micro evolution of Filecoin Miner ecosystem in specific areas of the world:

2. Micro view: where you can view specific areas in the world and analyze them. This is similar to zooming in google earth and viewing specific areas for network analysis.

- Similar to how are building it for FOAM : https://foamviz.surge.sh/#/poi-analytics
  - The difference is that the earth shows a macro view but removes the specifics from a UX perspective. The specific view helps you get into the nitty gritties for your analysis.

#### Marketing and Education deliverables:

##### 1. FILViz bot:

Bot that sends out weekly/bi-monthly/monthly updates in the form of a gif (eg) to build up interest on social media eg.
